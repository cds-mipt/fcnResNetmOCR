...
def crop_resize_v1(data_dir, save_dir, new_size=(1920, 1080)):
...
def crop_resize(data_dir, save_dir, new_size=(1920, 1080)):
    ...
    for image_name in tqdm(image_names):
        ...
        label = Image.open(label_path)
        w, h = image.size
        w_target, h_target = new_size
        ratio = w_target / h_target
        new_h = round(w / ratio)

        if new_h <= h:
            h_shift = random.randint((h - new_h) // 2, h - new_h)

            image = image.crop((0, h_shift, w, h_shift + new_h))
            label = label.crop((0, h_shift, w, h_shift + new_h))
        else:
            new_w = round(h * ratio)
            w_shift = random.randint((w - new_w) // 2, w - new_w)

            image = image.crop((w_shift, 0, w_shift + new_w, h))
            label = label.crop((w_shift, 0, w_shift + new_w, h))
